...
void Solver::sweep_ase_mod()
{
	std::fill(phi.begin(), phi.end(), 0.0);
	// use, e.g., ybs[0] because only ybs[-1] could be smaller
	// x-directed  Z         Y         z size       y size
	real bd_info_x[mesh.nbz][mesh.nby][mesh.zbs[0]][mesh.ybs[0]][n_eg];
	// y-directed  Z         X         z size       x size
	real bd_info_y[mesh.nbz][mesh.nbx][mesh.zbs[0]][mesh.xbs[0]][n_eg];
	// z-directed  Y         X         y size       x size
	real bd_info_z[mesh.nby][mesh.nbx][mesh.ybs[0]][mesh.xbs[0]][n_eg];
	...
	for (int o = 0; o < 8; o++)
	{
		...
		for(int a = 0; a < n_a; a++)
		{
			...
			SetValue(&bd_info_x[0][0][0][0][0],
					mesh.nbz*mesh.nby*mesh.zbs[0]*mesh.ybs[0]*n_eg, 0.0);
			SetValue(&bd_info_y[0][0][0][0][0],
					mesh.nbz*mesh.nbx*mesh.zbs[0]*mesh.xbs[0]*n_eg, 0.0);
			SetValue(&bd_info_z[0][0][0][0][0],
					mesh.nby*mesh.nbx*mesh.ybs[0]*mesh.xbs[0]*n_eg, 0.0);
...
#pragma omp simd
									for (int g = 0; g < n_eg; ++g)
									{
										//========================================================//
										// THE WORK!
										c[g] = ( muDelta[a] * bd_info_x[k_b][j_b][kk][jj][g] +
												    etaDelta[a] * bd_info_y[k_b][i_b][kk][ii][g] +
												     xiDelta[a] * bd_info_z[j_b][i_b][jj][ii][g] +
												     Q[cell * n_eg + g]) / (SigT[m * n_eg + g] + sum[a]);
										phi[cell * n_eg + g] += wt[a] * c[g];
										bd_info_x[k_b][j_b][kk][jj][g] = 2.0*c[g] - bd_info_x[k_b][j_b][kk][jj][g];
										bd_info_y[k_b][i_b][kk][ii][g] = 2.0*c[g] - bd_info_y[k_b][i_b][kk][ii][g];
										bd_info_z[j_b][i_b][jj][ii][g] = 2.0*c[g] - bd_info_z[j_b][i_b][jj][ii][g];
										//========================================================//
									} // GROUPS
								} // I
							} // J
